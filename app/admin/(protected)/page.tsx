      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-8">
        {[
          ...
          <div key={label} className={`rounded-2xl p-5 ${bg}`}>
            ...
            <div className={`text-sm font-semibold mt-1 ${text} opacity-70`}>{label}</div>
          </div>
        ))}
      </div>
          ...
          >
            ...
            <div className="p-5">
              <div className="flex items-center justify-between mb-3">
                <div>
                  <h2 className="font-bold text-gray-800 text-lg group-hover:text-violet-700 transition-colors">
                    {planta.nombre}
                  </h2>
                  <p className="text-gray-400 text-sm">{planta.habitaciones.length} habitaciones · {planta.precio_mensual} Bs./mes</p>
                </div>
                <div className="flex items-center gap-3">
                  ...
                  <ChevronRight size={18} className="text-gray-300 group-hover:text-violet-400 transition-colors flex-shrink-0" />
                </div>
              </div>
